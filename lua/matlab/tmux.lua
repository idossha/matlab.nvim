...
function M.capture_pane()
  ...
  local cmd = "capture-pane -p -t " .. vim.fn.shellescape(target)
  local success, output = pcall(M.execute, cmd)
...
function M.run_and_capture(command, skip_interrupt)
  local target = M.get_server_pane()
  if not target then
    return nil
  end
  ...
  M.execute("send-keys -t " .. vim.fn.shellescape(target) .. " 'clear' Enter")
  ...
  -- Send the command
  local cmd = vim.fn.escape(command, '"')
  M.execute("send-keys -t " .. vim.fn.shellescape(target) .. " " .. vim.fn.shellescape(cmd))
  M.execute("send-keys -t " .. vim.fn.shellescape(target) .. " Enter")
